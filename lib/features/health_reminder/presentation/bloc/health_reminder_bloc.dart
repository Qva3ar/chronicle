...
class HealthReminderBloc extends Bloc<HealthReminderEvent, HealthReminderState> {
  ...
  void _onHealthReminderDayOfWeekClicked(HealthReminderDayOfWeekButtonClicked event,
      Emitter<HealthReminderState> emit,) {
    final dayOfWeek = event.dayOfWeek;

    List<DayOfWeek> daysOfWeek = state.daysOfWeek;
    final newList = daysOfWeek.map((model) =>
    model == dayOfWeek
        ? model.copyWith(isSelected: event.dayOfWeek.isSelected ? false : true)
        : model).toList();

    emit(state.copyWith(daysOfWeek: newList));
  }

  void _onHealthReminderRepeatModeChanged(HealthReminderRepeatModeChanged event,
      Emitter<HealthReminderState> emit,) {
  ...
  void _onHealthReminderTimeSelected(HealthReminderTimeSelected event,
      Emitter<HealthReminderState> emit,) {
  ...
  void _onHealthReminderByIdLoaded(HealthReminderByIdLoaded event,
      Emitter<HealthReminderState> emit,) async {
    final id = event.id;
    if (id == null) {
  ...
  void _onHealthReminderSaveButtonClicked(HealthReminderSaveButtonClicked event,
      Emitter<HealthReminderState> emit,) async {
    final id = state.id;
