...
import '../../../health_center/presentation/models/data_stream.dart';
import '../bloc/home_bloc.dart';

class _Body extends StatefulWidget {
  _Body({Key? key, this.recordIds}) : super(key: key);
                    ...
                    child: GroupedListView<Record, DateTime>(
                      ...
                      itemBuilder: (context, dynamic item) {
                        ...
                        return GestureDetector(
                          onTap: () {
                            FocusManager.instance.primaryFocus?.unfocus();
                            final route = CardRoute.getRouteWithArgs(
                                item.id, item.title, item.text, item.tagIds, item.isGenerated);
                            print('ROUTE ==== >>>>> $route');
...
class HomePage extends StatelessWidget {
  HomePage({super.key, this.recordIds});

  String? recordIds = "";
  final streamData = DataStream();
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      ...
      child: BlocBuilder<HomeBloc, HomeState>(
        buildWhen: (curr, prev) => prev.isChanged != curr.isChanged,
...
class _FloatingActionButtons extends StatelessWidget {
  const _FloatingActionButtons(
      {required this.selectedChipIndex, required this.onTagSelected, required this.loadRecords});
      ...
      children: [
        ...
        Padding(
          ...
          child: FloatingActionButton(
            onPressed: () {
              final route = CardRoute.getRouteWithArgs(null, '', '', null, false);
              context.push(route).then((value) => loadRecords(refresh: true));
            },
